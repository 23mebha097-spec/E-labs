...
def main():
    ...
    def loss(x):
        robot.joints["joint_05_06"].current_value = x[0]
        robot.joints["joint_06_07"].current_value = x[1]
        robot.update_kinematics()
        tcp_pos = robot.get_tcp_world_pose(tcp_link)[:3, 3]
        return np.linalg.norm(tcp_pos - target_pos)
        
    # Minimize
    res = minimize(loss, x0=[180.0, 0.0], bounds=[(-180, 180), (0, 45)])
    print(res)
    print("Solved joint_05_06:", res.x[0])
    print("Solved joint_06_07:", res.x[1])
    
    robot.joints["joint_05_06"].current_value = res.x[0]
    robot.joints["joint_06_07"].current_value = res.x[1]
    robot.update_kinematics()
    tcp_pose = robot.get_tcp_world_pose(tcp_link)
    print("Resulting TCP pos (cm):", (tcp_pose[:3, 3] / 10.0).tolist())
